...
- `source_evalsets/` - the original ("generation 1") golden datasets:
  ...
  reference data, covering the original 169-document (English-only) corpus.
- `evalsets/` - newer eval sets, built from the manual doc-review process
  ...
  This is where new eval sets should be added going forward.
...
- `kp-docs/` - the underlying corpus documents (markdown with YAML
  frontmatter for the original 169 English docs, plus raw PDFs for some
  non-English docs).
...
The corpus grew from 169 English-only documents to a 207-document
cross-lingual corpus (English, Chinese, Spanish, Portuguese). New eval sets
are being built as follows:
...
2. An LLM (Claude Sonnet 5, via OpenCode) converts each doc-review entry
   into a matching pair of test cases - one in `evalsets/evalset_cite_NN.json`
   and one in `evalsets/evalset_answer_NN.json` - sharing the same `id` and
   `question`, so the two files stay 1:1 aligned per doc-review entry. This
   includes:
   - Resolving `external_id`/`document_id` for the source document, and any
     confirmed or suspected cross-lingual twin, using
   ...
   - **Writing the `canonical_answer` and `key_facts` fields for Answer
     ...
     notes (reformatted into prose / verbatim-ish bullets), not written
     directly by the human reviewer. No new facts are meant to be
     introduced beyond what's in the reviewer's notes, but **this content
   ...
   - `difficulty` and `query_type` are also LLM-inferred estimates (not
     supplied by the reviewer), pending human review.
...
- Replace a single `expected_document_ids` list (of `external_id` strings)
  ...
  same document, so they can be used together or independently. (Cite mode
  test cases carry these at the top level; Answer mode test cases carry
  them under `retrieval_ground_truth`, as before.)
- Add `source_document_id` and `source_language` - the UUID and language of
  ...
  drafted (useful for traceability, especially when a cross-lingual twin is
  also included as an expected match).
- Add an (currently empty) `task_description` field, kept for consistency
  with generation 1's schema but not populated yet.
...
- Do not (yet) include a bottom-level `metadata` summary block (test case
  counts, query_type list, changelog) as generation 1 did; may be added
  back later.
